...
$field = new Dhtechnologies_Ediconnectorbase_Model_Connector_Edi_Core_Documents_Type_EdiField($lineDefinition->columnDefinition->column[0]);


print $t->ok(is_object($field),"",false);

$value = $field->getPosition();
print $t->ok($value == 1,"",$value);

$value = $field->getFieldName();
print $t->ok($value == "Field1","",$value);

$field->setFieldName("Field9");

$value = $field->getFieldName();
print $t->ok($value == "Field9","",$value);

$value = $field->getType();
print $t->ok($value == "ID","",$value);

$field->setType("A");
$value = $field->getType();
print $t->ok($value == "A","",$value);


$value = $field->getMaxLength();
print $t->ok($value == 3,"",$value);

$field->setMaxLength(9);
$value = $field->getMaxLength();
print $t->ok($value == 9,"",$value);



$value = $field->isRequired();
print $t->ok($value ,"",$value);
