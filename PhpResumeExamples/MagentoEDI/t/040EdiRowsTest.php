...
//require_once("connector/edi/core/documents/type/EdiRow.php");
//require_once("connector/edi/core/documents/type/EdiRows.php");
...
$row = new Dhtechnologies_Ediconnectorbase_Model_Connector_Edi_Core_Documents_Type_EdiRow();
$rows = new Dhtechnologies_Ediconnectorbase_Model_Connector_Edi_Core_Documents_Type_EdiRows();

print $t->ok(is_object($row),"",false);
print $t->ok(is_object($rows),"",false);


$row->setValue("TEST", "999");

print $t->ok($row->getValue("TEST") == "999","1",$row->getValue("TEST"));
print $t->ok($row->getValue("TEST9") == null,"2",$row->getValue("TEST9"));

print $t->ok($rows->size() == 0,"",$rows->size());
print $t->ok($rows->getRow(0) == null,"",$rows->getRow(0));
print $t->ok($rows->getRow(1) == null,"",$rows->getRow(1));

$rows->addRow($row);

print $t->ok($rows->size() == 1,"",$rows->size());


$rout = $rows->getRow(0);
print $t->ok($rout->getValue("TEST") == "999","",$rout->getValue("TEST"));
